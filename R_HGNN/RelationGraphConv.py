...
class RelationGraphConv(nn.Module):
    ...
    def forward(self, graph: dgl.DGLHeteroGraph, feat: tuple, dst_node_transformation_weight: nn.Linear,
                src_node_transformation_weight: nn.Linear, relation_embedding: torch.Tensor,
                relation_transformation_weight: nn.Linear):
        r"""
        ...
        feat_src = torch.matmul(feat_src, src_node_transformation_weight.weight).view(-1, self._num_heads,
                                                                                      self._out_feats)
        # Tensor, (N_dst, n_heads, hidden_dim) -> (N_dst, input_dst_dim) * (input_dst_dim, n_heads * hidden_dim)
        feat_dst = torch.matmul(feat_dst, dst_node_transformation_weight.weight).view(-1, self._num_heads,
                                                                                      self._out_feats)
        ...
        relation_attention_weight = torch.matmul(relation_embedding.unsqueeze(dim=0),
                                                 relation_transformation_weight.weight).view(self._num_heads,
                                                                                             2 * self._out_feats)
